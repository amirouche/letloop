(library (letloop byter)
      ...
      (let ((out '()))
        (lambda (maybe-byte)
          (if (eof-object? maybe-byte)
              ;; TODO: optimize with a single pass over `out`
              (u8-list->bytevector (reverse out))
              (set! out (cons maybe-byte out)))))))
  ...
  (define check~byter-000
    (lambda ()
      (define expected (list 'null #t #f 0 #vu8(13 37) "az ul inu" 42 -101 (expt 2 32) (- (expt 2 32))))
      (assert (equal? (byter-unpack (apply byter-pack expected)) expected))))
