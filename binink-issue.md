...
After extensive investigation, this is **definitively a Chez Scheme bug/limitation** with R6RS library compilation instances in boot files, not a binink bug.

## Problem

When using `binink compile` to build standalone binaries from interdependent R6RS libraries:
...
## What Was Fixed in Binink

During investigation, several real bugs were found and fixed in binink:
...
```
**Fix**: `binink-discover-libraries` now deduplicates by library name.
...
SCHEME=/src/binink/local/bin/scheme binink compile ./ lib-b.sls main
...
4. Embeds resulting program.boot into C binary via `binink-program.c`
...
6. ✅ Clean ALL .so/.wpo files (both binink and target project)
...
- Investigation: `/src/binink/binink-issue.md` (this file)
- Fixed code: `/src/binink/src/binink/base.scm` (binink-compile function)
