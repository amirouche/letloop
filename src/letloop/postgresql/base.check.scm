;; Checks for (letloop postgresql base).  The crypto and wire-encoding
;; checks are pure and always run.  The connection checks want a
;; PostgreSQL listening on 127.0.0.1:5432 (the CI service provides
;; one); on a machine without it they print a SKIP note and pass, so
;; `make check` stays useful everywhere.
...
(define ~check-postgresql-query-000
  ...
  (lambda ()
    (with-pg conn
      (let ((result (pg-query conn "SELECT 1 AS n")))
...
(define ~check-postgresql-error-000
  ...
  (lambda ()
    (with-pg conn
      (let ((result (pg-query conn "NOT VALID SQL")))
...
(define ~check-postgresql-null-000
  ...
  (lambda ()
    (with-pg conn
      (let ((result (pg-query conn "SELECT NULL AS v")))
        (assert (not (pg-result-error? result)))
...
(define ~check-postgresql-multirow-000
  ...
  (lambda ()
    (with-pg conn
      (let ((result (pg-query conn "SELECT n FROM generate_series(1,3) AS n")))
        (assert (not (pg-result-error? result)))
