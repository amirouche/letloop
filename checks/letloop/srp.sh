...
if [ ! -d "$CHECKSDIR/.venv" ]; then
    echo "Creating Python venv and installing srp..."
    uv venv "$CHECKSDIR/.venv"
    uv pip install --python "$CHECKSDIR/.venv/bin/python" srp
fi

# --- Python side ---
echo "Running pysrp..."
uv run --python "$CHECKSDIR/.venv/bin/python" "$CHECKSDIR/srpcheck.py" > "$TMPDIR/python.txt" 2>"$TMPDIR/python_err.txt"
