...
letloop: clean src/letloop-program.c src/letloop-usage.md src/letloop/base.scm ## Produce letloop.boot from letloop/base.scm's letloop-main, and install it
	...
	@# bin/letloop is a wrapper rather than a symlink, and the `--` it
	@# inserts is the whole point: because the binary *is* the scheme
	@# executable, Chez's C main parses argv before any Scheme runs and
	@# claims --help, --version, --optimize-level, --libdirs and a dozen
	@# more, at ANY position on the line -- `letloop check --version`
	@# printed Chez's version. `--` ends that parsing, and everything
	@# after it reaches scheme-start untouched. The path it execs stays
	@# relative so the install tree can still be moved, and the target
	@# keeps the basename `letloop` so it still finds letloop.boot.
	@#
	@# Only this one path needs it. `letloop compile` links
	@# src/letloop-program.c, whose main calls Sscheme_start(argc, argv)
	@# directly, so compiled programs never see Chez's parser.
	BOOT=$$(dirname $$(readlink -f $(SCHEME))); \
	  install -m 644 letloop.boot "$$BOOT/letloop.boot"; \
	  ln -f "$$BOOT/scheme" "$$BOOT/letloop"; \
	  mkdir -p $(PREFIX)/bin; \
	  REL=$$(realpath --relative-to="$(PREFIX)/bin" "$$BOOT/letloop"); \
	  rm -f $(PREFIX)/bin/letloop $(PREFIX)/bin/letloop.tmp; \
	  printf '%s\n' \
	    '#!/bin/sh' \
	    '# Generated by `make letloop`. letloop is the Chez Scheme binary' \
	    '# under another name, so Chez'\''s C main would otherwise claim' \
	    '# --help, --version, --optimize-level and more, at any position.' \
	    '# `--` ends Chez'\''s option parsing; the rest reaches letloop.' \
	    '# No command substitution here: `readlink -f` and `dirname` are a' \
	    '# fork each, and two forks measured 3.4ms against 1.0ms for the' \
	    '# parameter expansion below -- on a binary whose whole startup is' \
	    '# 32ms. The cost is that a symlink TO this wrapper is not resolved,' \
	    '# so the exec below would look beside the symlink. Install it, do' \
	    '# not link to it.' \
	    'd=$${0%/*}; [ "$$d" = "$$0" ] && d=.' \
	    'exec "$$d/'"$$REL"'" -- "$$@"' \
	    > $(PREFIX)/bin/letloop.tmp; \
	  chmod 755 $(PREFIX)/bin/letloop.tmp; \
	  mv -f $(PREFIX)/bin/letloop.tmp $(PREFIX)/bin/letloop; \
